...
local PlayersDropDown = TrollingSection:NewDropdown("Freeze Player", "DropdownInf", GetPlayers(), function(currentOption)
    plrToFreeze = SplitString(currentOption, "@")[1]
end)
...
TrollingSection:NewButton("Freeze Player", "Freezes the player", function()
    if plrToFreeze ~= "None" then 
       ...
       pcall(function()
        local A_1 = game:GetService("Players"):FindFirstChild(SplitString(plrToFreeze, " ")[1]).Character.HumanoidRootPart
        local A_2 = CFrame.new(math.random(-200,200), math.random(-200,200), math.random(-200,200))
        local Event = plr.Character.Slice.SliceEvent
        Event:FireServer(A_1, A_2)
        task.wait(0.2)
...
SpoofSection:NewDropdown("AFK Spoof Options", "DropdownInf", {"Spoof AFK", "Spoof Non-AFK"}, function(currentOption)
